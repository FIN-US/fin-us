...
- **API 정적 키 인증(#266 2단계)은 `FINUS_API_KEY`를 채운 배포에서만 걸립니다.** 채우면 `/api/` 아래 모든 요청이 `X-API-Key` 헤더를, `/api/v1/ws` 핸드셰이크가 `?api_key=...` 쿼리 파라미터를 요구합니다(브라우저 WebSocket API는 커스텀 헤더를 붙이지 못해 전달 경로가 다릅니다). 검사는 라우팅보다 먼저 걸리므로 존재하지 않는 `/api/` 경로도 401이라 엔드포인트 목록을 훑을 수 없고, `/health`는 접두사 밖이라 열려 있습니다(compose 헬스체크가 `curl`로 부릅니다). 비우면(기본값) 인증이 꺼지고 backend 기동 로그에 경고가 남습니다. **⚠️ 지금 켜면 Unity WebGL 대시보드가 401로 멈춥니다** — 추적 중인 번들의 `ApiClient`가 이 헤더를 붙이지 않고, 붙이려면 WebGL 재빌드가 필요합니다. 그래서 현재 켜는 것이 의미 있는 구성은 대시보드를 쓰지 않는 배포(텔레그램·NAT 중심 운용)이고, 번들이 키를 싣는 작업은 #266에 남아 있습니다.
