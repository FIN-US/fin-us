...
- **API 인증이 기본값에서는 꺼져 있음** — `FINUS_API_KEY`를 채우면 `/api/`와 `/api/v1/ws`에 정적 키 인증이 걸립니다(#266 3단계로 대시보드도 함께 통과합니다). 비워 둔 기본값에서는 `/api/` 전체가 무인증이므로 외부에 포트를 열지 마세요. 켜더라도 그 키는 브라우저에 도달하므로 **대시보드를 열 수 있는 사람은 API도 부를 수 있습니다** ([#266](https://github.com/FIN-US/fin-us/issues/266))
