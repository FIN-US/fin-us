...
- `[delivery-fail] kind=... count=...` — 사용자가 받지 못한 메시지 한 건마다 한 줄. `kind`는 `settled_send`(주문 프롬프트·취소 확인·분석 답변과 분석·실적 조회·주문 보조 실패 통지·자동 제안 승인 요청, 버튼 응답 실패 시 대신 보내는 만료·지원하지 않는 버튼 안내 등), `fill_notify`(체결 통지 첫 전송), `fill_redelivery`(체결 통지 재배달 — 1분마다 다시 시도해도 **체결 한 건당 한 번**), `fill_mark`(전송 후 통지 완료 기록 실패 — 같은 통지가 한 번 더 나가므로 발생마다)입니다.
