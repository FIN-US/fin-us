...
- `/api/`에는 nginx가 레이트리밋을 겁니다(#266 1단계). `/api/v1/analyze`는 **기본값에서 인증이 없으면서**(아래 API 키 항목 참고) 호출 한 번이 LLM 또는 NAT 멀티 에이전트를 태워 직접 과금으로 이어지므로 6r/m(burst 2) + **동시 2건**으로 따로 낮게 잡고, 그 밖의 `/api/`·`/health`는 2r/s(burst 20)입니다. 정적 자산과 `/nginx-health`는 제한 대상이 아닙니다. 초과 응답은 429이며 본문이 `{"detail": ...}` JSON이라 Unity 배너에 그대로 읽힙니다 — 레이트 초과는 `Retry-After: 10`과 함께, 동시 실행 초과는 그 헤더 없이(언제 풀릴지 서버가 알 수 없습니다) 서로 다른 문구로 나갑니다. **스케줄러의 자동 분석은 여기 걸리지 않습니다** — `perform_stock_analysis`를 같은 프로세스에서 직접 부르고 HTTP를 타지 않습니다. 이 제한은 비용의 뚜껑이지 접근 제어가 아닙니다 — 접근 제어는 아래 API 키 항목(#266 2단계)이 맡습니다.
- **API 정적 키 인증(#266 2단계)은 `FINUS_API_KEY`를 채운 배포에서만 걸립니다.** 채우면 `/api/` 아래 모든 요청이 `X-API-Key` 헤더를, `/api/v1/ws` 핸드셰이크가 `?api_key=...` 쿼리 파라미터를 요구합니다(브라우저 WebSocket API는 커스텀 헤더를 붙이지 못해 전달 경로가 다릅니다). 검사는 라우팅보다 먼저 걸리므로 존재하지 않는 `/api/` 경로도 401이라 `/api/`를 두드려 엔드포인트를 찾아낼 수 없습니다. 다만 `/openapi.json`·`/docs`·`/health`는 접두사 밖이라 인증을 켜도 열려 있습니다 — 8080에서는 nginx가 404로 끝내지만 **8000에 직접 닿으면 전체 스키마가 그대로 나옵니다.** 스키마는 저장소 소스 그 자체이고 8000은 루프백 전용이라(#246) 함께 닫지 않았습니다. 즉 이 인증이 지키는 것은 "무엇이 있는지"가 아니라 "무엇을 호출할 수 있는지"입니다. **WebSocket 키는 URL에 실려 nginx 액세스 로그에 평문으로 남습니다.** 비우면(기본값) 인증이 꺼지고 backend 기동 로그에 경고가 남습니다. **⚠️ 지금 켜면 Unity WebGL 대시보드가 401로 멈춥니다** — 추적 중인 번들의 `ApiClient`가 이 헤더를 붙이지 않고, 붙이려면 WebGL 재빌드가 필요합니다. 그래서 현재 켜는 것이 의미 있는 구성은 대시보드를 쓰지 않는 배포(텔레그램·NAT 중심 운용)이고, 번들이 키를 싣는 작업은 #266에 남아 있습니다.
...
- **API 인증이 기본값에서는 꺼져 있음** — `FINUS_API_KEY`를 채우면 `/api/`와 `/api/v1/ws`에 정적 키 인증이 걸리지만, 지금 켜면 Unity WebGL 대시보드가 키를 싣지 못해 401로 멈춥니다(번들 재빌드 필요). 대시보드를 쓰는 배포라면 외부에 포트를 열지 마세요 ([#266](https://github.com/FIN-US/fin-us/issues/266))
