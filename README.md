...
- `/api/`에는 nginx가 레이트리밋을 겁니다(#266 1단계). `/api/v1/analyze`는 **인증을 끈 배포에서는 무인증이면서**(아래 API 키 항목 참고) 호출 한 번이 LLM 또는 NAT 멀티 에이전트를 태워 직접 과금으로 이어지므로 6r/m(burst 2) + **동시 2건**으로 따로 낮게 잡고, 그 밖의 `/api/`·`/health`는 2r/s(burst 20)입니다. 정적 자산과 `/nginx-health`는 제한 대상이 아닙니다. 초과 응답은 429이며 본문이 `{"detail": ...}` JSON이라 Unity 배너에 그대로 읽힙니다 — 레이트 초과는 `Retry-After: 10`과 함께, 동시 실행 초과는 그 헤더 없이(언제 풀릴지 서버가 알 수 없습니다) 서로 다른 문구로 나갑니다. **스케줄러의 자동 분석은 여기 걸리지 않습니다** — `perform_stock_analysis`를 같은 프로세스에서 직접 부르고 HTTP를 타지 않습니다. 이 제한은 비용의 뚜껑이지 접근 제어가 아닙니다 — 접근 제어는 아래 API 키 항목(#266 2·3단계)이 맡습니다.
- **API 정적 키 인증(#266)은 `FINUS_API_KEY`를 채운 배포에서 걸리고, `bash scripts/setup_env.sh`로 `.env`를 처음 만들면 난수 키가 채워져 켜진 채 시작합니다.** 이미 있는 `.env`는 비어 있어도 채우지 않으므로, 그 전부터 쓰던 배포는 직접 채워야 켜집니다. Unity 에디터 플레이 모드는 8000에 직접 붙어 아래 쿠키를 받지 못하므로, 에디터로 테스트하는 동안에는 값을 비우세요. 채우면 `/api/` 아래 모든 요청과 `/api/v1/ws` 핸드셰이크가 이 키를 요구합니다. 키가 서버에 닿는 경로는 둘입니다 — 비브라우저(`curl`·`wscat`·NAT)는 `X-API-Key` 헤더를 직접 싣고, 브라우저는 `frontend`의 nginx가 문서 응답에 실어 준 `finus_api_key` 쿠키(`Path=/; SameSite=Strict; HttpOnly`)를 이후의 요청과 WebSocket 핸드셰이크에 자동으로 붙입니다(#266 3단계). 그래서 **키를 채워도 Unity WebGL 대시보드가 그대로 동작합니다 — 번들 재빌드가 필요 없습니다.** `SameSite=Strict`는 장식이 아니라 이 방식의 CSRF 방어이고, 임의 페이지의 `fetch(..., { mode: "no-cors" })` 경로까지 함께 닫습니다(CORS로는 막지 못했던 그 경로입니다). 단 여기서 "site"는 스킴 + 등록가능도메인이고 **포트는 보지 않으므로**, 같은 호스트의 다른 포트에 뜬 페이지는 same-site라 쿠키를 그대로 싣습니다 — 지금 전 인터페이스에 게시되는 서비스가 `frontend`(8080) 하나뿐이라(#246·#285) 실제로 그런 페이지가 없을 뿐입니다. 검사는 라우팅보다 먼저 걸리므로 존재하지 않는 `/api/` 경로도 401이라 `/api/`를 두드려 엔드포인트를 찾아낼 수 없습니다. 다만 `/openapi.json`·`/docs`·`/health`는 접두사 밖이라 인증을 켜도 열려 있습니다 — 8080에서는 nginx가 404로 끝내지만 **8000에 직접 닿으면 전체 스키마가 그대로 나옵니다.** 스키마는 저장소 소스 그 자체이고 8000은 루프백 전용이라(#246) 함께 닫지 않았습니다. 즉 이 인증이 지키는 것은 "무엇이 있는지"가 아니라 "무엇을 호출할 수 있는지"입니다. **⚠️ 페이지를 열 수 있는 사람은 API도 부를 수 있습니다** — 정적 키가 브라우저에 도달하는 순간 성립하는 성질이라 키를 어떤 방식으로 들여보내도 같고, 없애려면 로그인이나 네트워크 레벨 인증이 필요합니다(이 배포에서 브라우저 경계를 맡는 것은 Tailscale입니다). 비우면 인증이 꺼지고 backend 기동 로그에 경고가 남습니다. 키를 바꾸면 **`frontend` 컨테이너도 다시 만들어야** 합니다 — nginx는 기동 시점에 한 번만 치환합니다(`docker compose up -d backend frontend`).
...
- `backend` 컨테이너는 `--reload` 없이 뜹니다(#396). 소스는 `.:/app`로 마운트돼 있으므로 코드를 고친 뒤 `docker compose restart backend`로 반영합니다. 저장할 때마다 자동으로 다시 띄우려면 `docker-compose.override.example.yml`을 `docker-compose.override.yml`로 복사하세요 — compose가 자동으로 병합하고, 복사본은 `.gitignore`에 등록돼 있습니다. 리로드는 스케줄러·텔레그램 폴러까지 함께 재시작하므로 개발할 때만 씁니다.
...
<a id="telegram-delivery-failure" name="telegram-delivery-failure"></a>

### 전송 실패 신호

주문·체결 통지 경로의 텔레그램 전송이 끝내 실패하면 두 종류의 줄이 backend 로그에 남습니다(#259).

- `[delivery-fail] kind=... count=...` — 사용자가 받지 못한 메시지 한 건마다 한 줄. `kind`는 `settled_send`(주문 프롬프트·취소 확인·분석 답변과 분석·실적 조회·주문 보조 실패 통지·자동 제안 승인 요청, 버튼 응답 실패 시 대신 보내는 만료·지원하지 않는 버튼·주문 저장소 오류 안내 등), `fill_notify`(체결 통지 첫 전송), `fill_redelivery`(체결 통지 재배달 — 1분마다 다시 시도해도 **체결 한 건당 한 번**), `fill_mark`(전송 후 통지 완료 기록 실패 — 같은 통지가 한 번 더 나가므로 발생마다)입니다.
...
```bash
docker compose logs -f --no-log-prefix backend | grep -E '\[delivery-(fail|alarm)\]'
```
...
| `/confirm` | 대기 중인 주문 실행 (자동 제안 주문은 확정 버튼으로만) | - |
| `/cancel` | 대기 중인 주문 취소 (증권사 제출 주문은 취소 불가) | - |

> 종목명이 공백 뒤 숫자로 끝나는 종목(`KODEX 200`·`TIGER 200` 등)도 이름 그대로 입력할 수 있습니다(`/buy KODEX 200 10`). 끝의 두 값이 모두 숫자면 "지정가 주문"과 "종목명의 일부 + 시장가 주문" 두 가지로 읽힐 수 있어, 봇이 종목 마스터에서 실제로 있는 종목명 쪽을 고릅니다. 두 해석이 모두 실재 종목이면 추측하지 않고 주문을 만들지 않은 채 두 해석과 각각의 6자리 종목코드 명령(`/buy 069500 10` 등)을 보여 줍니다([#387](https://github.com/FIN-US/fin-us/issues/387)).
...
`/confirm`이 체결되면 봇이 `주문 완료: …`를 보냅니다. 이 통지는 **전송에 실패해도 사라지지 않습니다**(#259). 체결은 먼저 `TradeHistory`에 기록되고, 통지가 나가야 그 행의 `notified_at`이 채워집니다. 비어 있는 행은 1분 주기 `trade_notification` 잡이 다시 알립니다 — 그때는 재전송임을 밝히는 문구가 붙습니다.

- 재전송 대상은 체결 후 1분이 지났고 24시간이 지나지 않은 미통지 체결입니다. 며칠 지난 체결을 지금 알리는 것은 복구가 아니라 소음이라 창을 닫아 뒀습니다.
- 이미 받은 통지가 한 번 더 올 수 있습니다(통지는 나갔는데 기록 직전에 프로세스가 죽은 경우). 주문이 두 번 나간 것이 아니며, 재전송 문구가 그것을 밝힙니다.
- 체결 이력 기록 자체가 실패하면 재전송할 근거가 없습니다. 그때는 봇이 그 자리에서 재시도하고, 메시지에 "이 메시지가 유일한 통지입니다"라고 적습니다.
...
자동 제안 주문은 **확정 버튼으로만** 확정됩니다. `/confirm`을 보내면 실행하지 않고 버튼을 누르라고 안내합니다(#390). 다른 주문을 보고 보낸 `/confirm`이 기기의 전송 지연(오프라인 대기 등) 끝에 서버에 늦게 도착하면, 그사이 생긴 자동 제안을 확정한 것으로 읽힐 수 있기 때문입니다. 버튼은 주문마다 다른 값에 묶여 있어 이 문제가 없습니다. 직접 낸 `/buy`·`/sell`·자연어 주문·`/advise`는 지금처럼 `/confirm`으로도 확정됩니다.
...
- **이전부터 쓰던 `.env`로는 API 인증이 꺼져 있을 수 있음** — 새 설치는 `setup_env`가 난수 키를 채워 켜진 채 시작하지만, 이미 있는 `.env`는 건드리지 않습니다. `FINUS_API_KEY`를 채우면 `/api/`와 `/api/v1/ws`에 정적 키 인증이 걸리고 대시보드도 함께 통과합니다. 비어 있으면 `/api/` 전체가 무인증이므로 외부에 포트를 열지 마세요(backend 기동 로그에 경고가 남습니다). 켜더라도 그 키는 브라우저에 도달하므로 **대시보드를 열 수 있는 사람은 API도 부를 수 있습니다** ([#266](https://github.com/FIN-US/fin-us/issues/266))
