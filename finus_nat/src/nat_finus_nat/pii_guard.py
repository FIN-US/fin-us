...
def restore_params_for_kis(
    ...
    **이 함수가 막는 것은 종류 *간* 맞바꿈뿐이다.** 같은 종류 안에서 값이 뒤바뀌는 것
    (평가금액 자리표시자를 ``ORD_UNPR``에 싣는 등)은 둘 다 ``AMOUNT``라 이 함수를
    그대로 통과한다. 자리표시자는 숫자의 타당성 신호를 지우므로 — 평문이었다면
    "주가가 1,234만원"에서 LLM이 느꼈을 이상함이 사라진다 — 이 계층이 만든 위험이
    맞지만, 값이 의미상 맞는지는 종류가 아니라 종목의 현재가와 비교해야 하는 별도
    판정이다. 그 판정은 KIS로 보내기 직전 :mod:`~nat_finus_nat.order_price_guard`가
    backend 주문 보조와 같은 지정가 괴리 규칙으로 코드가 내린다(#365).
