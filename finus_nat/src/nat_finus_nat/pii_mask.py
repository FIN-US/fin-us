...
LLM이 수행할 수 있어 유지되지만, 여기서 비교되는 것은 **번호가 아니라 값**이다 —
번호(_1, _2)는 값의 동일성 표지일 뿐이고 텍스트 등장 순서를 뜻하지 않는다. mask_pii가
정규식별로 한 번씩 훑으므로 번호는 훑는 순서를 따른다("3000원 그리고 1만5000원"의
AMOUNT_1은 뒤에 있는 "1만5000원"이다). 근거와 실측은 mask_pii의 순서 주석과
test_unit_and_bare_won_amounts_coexist_losslessly에 있다.

반면 **절대 금액 기반 판단**("이 종목 비중이 1천만원으로 과도하다")은
원값이 프롬프트에 없으므로 LLM이 할 수 없다. 이것은 이 설계의 알려진 한계이며,
...
# 계속 매치하지 않고 _AMOUNT_WON_RE가 가져간다.
#
# 꼬리 앞 공백 (?:(?<=[만억])[ \t])? — "1만 5000원"처럼 단위 뒤에 공백을 두는 표기.
# 붙임 표기와 같은 값이고 오히려 자유 입력(_handle_chat_fallback)에서 더 흔하다.
# 공백을 무조건 허용하지 **않고** 직전 문자가 "만"/"억"일 때로 좁히는 것이 요점이다.
# "조"를 넣으면 법조문 서수사에서 새 과탐이 생긴다 — "제3조 5000원 이하의 과태료"가
# "제<AMOUNT_1> 이하의 과태료"(매핑 "3조 5000원")가 되어 조문 번호까지 값에 삼켜진다.
# 아래 "제N조" 과탐과 같은 급이지만 그쪽은 \s?원이 한 음절을 먹는 것이고 이쪽은 별개
# 금액과 조문 번호를 하나로 합치는 것이라, 얻는 것("1조 5000원")보다 잃는 것이 크다.
# 그래서 "조" 뒤 공백 표기는 docs/nfr-05-pii-masking.md 미적용 경로 5번의 알려진
# 간극으로 남긴다("1조 5000원" -> "1조 <AMOUNT_1>").
#
# 공백 문자를 \s가 아니라 [ \t]로 한정하는 이유는 _QTY_UNIT_SUFFIX의 배제 목록과 같다 —
# \s는 개행을 넘는다. 실측으로 차이가 나는 것은 개행 케이스뿐이고(\s: "잔고는 1만\n
# 5000원"이 통째로 한 자리표시자가 된다), 금액 한 값이 줄을 넘어 쪼개지는 표기는 없다.
# 줄 끝의 "1만"과 다음 줄 첫머리의 "5000원"은 서로 다른 값일 가능성이 높고, 그 둘을 한
# 자리표시자로 합치면 왕복은 무손실이어도 LLM은 두 값이 있었다는 사실을 잃는다.
#
# 여전히 남는 것: 공백 뒤가 순수 숫자가 아니라 수사·단위를 가진 마디인 표기
# ("3억 2천만원"·"1억 5,000만원"·"9조 1800억원")는 꼬리가 숫자만 받으므로 그대로
# 뒤 구간만 잡힌다. 같은 미적용 경로 5번 항목에 있다.
