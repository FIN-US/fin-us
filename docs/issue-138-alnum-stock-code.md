...
**이 장치의 한계**: "두 계층이 같은 정의에서 나온다"는 표현은 실제로 산 것보다 강하다. 표가 고정하는 것은 **12개 코드에 대한 두 계층의 판정 일치**이지 입력 공간 전체의 동치가 아니다. 두 정규식이 이 12행에서만 같고 다른 입력에서 갈리는 변경(예: 한쪽에만 `/i`를 붙이면서 표에 없는 대소문자 조합을 여는 것)은 표를 통과할 수 있다. 표에 소문자 행(`0001a0`)·전각 숫자 행·공백 행을 둔 것은 실제로 갈릴 법한 축을 대표 입력으로 미리 집어넣어 그 틈을 줄이려는 것이지 없애는 것이 아니다. 축이 늘어나면(§6.5의 단계별 열) 행도 함께 늘려야 한다.
