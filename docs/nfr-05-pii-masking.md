...
UX 저하는 인정한다 — 사용자는 저장이 왜 실패했는지 알기 어렵다. 오류의 `hint`에
**원인과 조치**(개인정보 보호 처리로 값이 가려졌으니 일지에 넣을 값을 한 번 더 적어
달라)를 담고, 에이전트에게 같은 내용으로 재시도하지 말라고 명시해 완화한다.
...
"일지에 금액을 못 쓴다"가 실제로 자주 걸리면 1번(매핑 수명의 요청 범위 승격)을 별도
이슈로 올린다.
...
- **도구 인자 방향에는 역치환이 없다 (#338)** — 마스킹은 도구 **결과**에만 걸리고,
  ...
  선택이다 — **#338로 분리했다.**
- **사용자 발화 유래 자리표시자가 일지 DB에 남는다 (#339)** — **손상은 막았다. 근본
  해결은 아니다. 위 "저장 거부 가드" 절 참고 (#339).**
