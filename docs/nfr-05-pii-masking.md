...
등록소이고 매핑 자체는 여전히 호출마다 별개다. 정확히는 **scope가 충돌하지 않는 한**
그렇다 — 충돌하면 나중 등록이 앞선 것을 덮지만, 조회가 scope와 자리표시자 전체 문자열을
함께 맞춰야 성공하므로 결과는 틀린 값이 아니라 **저장 거부**다(이미 감수한 실패 모드와
같은 것이다). 매핑을 전역 dict **하나**로 올리는 것이
가장 짧은 "요청 범위 승격"인데, 그러면 겹쳐 도는 두 요청 중 나중 것이 앞선 것을 덮어
**다른 대화의 금액이 남의 일지에 저장된다.** `backend/tests/test_pii_registry.py`의
`TestConcurrentRequests`가 그 회귀만 골라 빨간불로 만든다(돌연변이로 실측했다).
