...
`_FALLBACK_LABEL`의 중립 문구("이전에 언급된 금액" 등)로 치환한 뒤 경고 로그만 남긴다.
...
5. **`원` 접미사 없는 만/억 단위 금액 및 숫자-단위 간 공백** — `"1억 5,000만원"`처럼
   앞에 단독 `만`/`억` 표기가 선행할 때, `_AMOUNT_UNIT_RE` 패치 후에도 `"1억 <AMOUNT_1>"`
   으로 앞의 `"1억"`이 원문에 남는다. `원` 접미사 없는 순수 `만`/`억` 표기는 세 정규식
   중 어느 것도 다루지 않기 때문이다. 추가로 `"2,000 만원"`처럼 숫자와 단위 사이에 공백이
   있는 표기도 마스킹되지 않는다(`_AMOUNT_UNIT_RE`의 `\s?`는 만/억과 원 사이에만 있고
   숫자와 단위 사이 공백은 다루지 않는다). 두 경우 모두 이번 PR에서 고치지 않고 알려진
   간극으로 남긴다.
