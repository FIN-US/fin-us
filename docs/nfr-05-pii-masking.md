...
### 저장 거부 가드 — 되돌릴 수 없으면 저장하지 않는다 (#339)

`restore_for_internal`은 **이 요청의 박스에 있는 것만** 되돌리고 나머지는 손대지 않는다.
위 "낯선 scope는 backend가 판정" 규칙을 저장 방향에도 그대로 적용한 것인데, 저장 경로에
대해서는 그 규칙이 **성립하지 않는다.**
...
통과 규칙의 전제는 "그 값이 backend로 **돌아간다**"인데, **저장은 backend 왕복의 바깥**이다.
요청이 끝나면 backend의 매핑은 `llm_chat` 지역 변수와 함께 사라져 복구 경로가 없다.
#230이 막으려던 것과 정반대의 손상이다 — 유출이 아니라 **원본 소실**이다.

`finus_save_diary`는 역치환한 제목·본문을 `unrestorable_placeholders`에 한 번 더 태워,
남은 자리표시자가 하나라도 있으면 **POST하지 않고** 오류를 돌려준다. 남을 수 있는 것은
두 종류이고 저장 관점에서 차이가 없다 — 둘 다 원값이 이 프로세스 어디에도 없다.

| 역치환 후에도 남은 자리표시자 | 출처 | 저장 |
| --- | --- | --- |
| 낯선 scope | backend가 마스킹한 사용자 발화(#230), 이전 턴 | **거부** |
| 이 요청 scope인데 박스에 없음 | LLM이 번호를 지어냄, 박스 유실 | **거부** |
| 박스에 있음 | 이 요청의 도구 결과 | 원값으로 되돌려 저장 |

#### 왜 근본 해결이 아니라 거부인가

이슈가 제시한 세 방향 중 **3(NAT이 저장 거부)** 을 택했다.

1. **backend `create_db_diary`가 저장 직전 `unmask_pii`** — 근본 해결이 맞다. 다만 그
   요청의 매핑이 `llm_chat` 지역 변수라 **수명을 요청 범위로 끌어올리는 구조 변경**이
   선행이고, 그 크기가 이 이슈를 넘는다.
2. **저장을 `llm_chat` 왕복 안으로** — 구조는 가장 깨끗하지만 배선 변경이 가장 크다.
3. **거부** — 지금 피해가 "조용한 영구 원본 소실"이고 복구 경로가 없다. 이런 종류는
   근본 해결보다 **먼저 시끄러운 실패로 바꾸는 것**이 순서다. 손상된 데이터가 DB에
   들어가는 것 자체를 막는다.

#### UX 저하 — 재입력으로는 회복되지 않는다

**사용자가 값을 다시 적어도 저장은 다시 실패한다.** `llm_chat`은 요청마다
`mask_pii(user_msg)`를 돌리므로(`backend/services.py`), 재입력한 금액은 **새 scope로 다시
마스킹돼** 에이전트에 또 자리표시자로 도착한다. "300만원이야" / "삼백만원" /
"3,000,000원 이었어" 모두 마스킹되는 것을 실측했다 — 단위 없는 맨숫자("300")를 빼면
평문으로 빠져나가는 표기가 없고, 그것은 사용자에게 시킬 만한 조치가 아니다.

그래서 `hint`는 재입력을 권하지 않는다. 대신 **재질의를 금지하고** 실제로 성립하는 두
경로를 준다: (1) 해당 금액·수량을 본문·제목에서 빼고 저장한다, (2) 이 요청의 잔고·주문
조회 도구 결과에서 나온 값만 써서 저장한다(그 값은 이 요청의 박스에 있어 되돌려진다).
저장 후 사용자에게 금액을 그대로 옮기지 못한 이유를 알리게 한다.

즉 **"사용자 발화에 적은 금액을 일지에 남기는 것"은 이 가드로 회복되지 않는다.** 그
기능을 되살리려면 방향 1(backend 마스킹 매핑의 수명을 요청 범위로 승격)이 선행이어야
한다. 이 절이 바꾼 것은 손상의 종류다 — 조용한 영구 소실에서, 사용자가 즉시 아는 실패로.

오류 JSON에는 자리표시자 **원문을 싣지 않는다**(종류와 개수만 싣는다). 오류는
Observation으로 LLM 컨텍스트에 재유입되므로, 토큰을 실으면 에이전트가 그것을 답변에
옮겨 적을 여지를 준다 — 지금 막으려는 것과 같은 종류의 오염이다. 원문은 서버 로그에만
남긴다.

1번(매핑 수명의 요청 범위 승격)은 **선택이 아니라 남은 숙제**이며 #354로 등록했다. 위 "재입력으로는
회복되지 않는다"가 그 이유이며, 사용자 발화의 금액을 일지에 남기는 기능 자체가 그때까지
복구되지 않는다.
...
3. **원값이 이 요청의 박스에 있음** — 낯선 scope(backend가 사용자 발화를 마스킹한
   것)나 LLM이 지어낸 번호는 원값이 이 프로세스 어디에도 없다. 저장 거부 가드가
   다루는 것과 같은 두 종류다.
...
- **도구 인자 방향에는 역치환이 없다 (#338)** — **해소됨. 위 "도구 인자 방향 —
  ...
  않는다 — 아래 새 항목 참고.
- **사용자 발화 유래 자리표시자가 일지 DB에 남는다 (#339)** — **손상만 막았다.** 저장을
  거부해 원본 소실은 끝냈지만, 사용자 발화의 금액을 일지에 남기는 것은 여전히 안 된다 —
  재입력해도 다시 마스킹되기 때문이다. 회복하려면 backend 마스킹 매핑의 수명을 요청
  범위로 승격해야 하며, **#354로 분리했다.** 위 "저장 거부 가드" 절 참고.
