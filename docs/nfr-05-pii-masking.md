...
5. **`원` 접미사 없는 만/억 단위 금액** — `"1억 5,000만원"`처럼 앞에 단독 `만`/`억`
   표기가 선행할 때, `_AMOUNT_UNIT_RE` 패치 후에도 `"1억 <AMOUNT_1>"`으로 앞의 `"1억"`이
   원문에 남는다. `원` 접미사 없는 순수 `만`/`억` 표기는 세 정규식 중 어느 것도 다루지
   않기 때문이다. 이번 PR에서 고치지 않고 알려진 간극으로 남긴다.
