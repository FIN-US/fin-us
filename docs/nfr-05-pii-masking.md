...
5. **한글 수사 체인 패치(`3a78eb4`, #230 5차 리뷰) 이후에도 남는 금액 표기 간극** —
   `_AMOUNT_UNIT_RE`에 숫자-만/억 사이 한글 수사(천/백/십) 갈래가 추가되어
   `"3천만원"`·`"1천5백만원"`·`"1천억원"`처럼 단일 단위 구간에 수사가 낀 표기는 이제
   **값 전체가** 마스킹된다(실측 확인). 다만 다음 세 형태는 패치 후에도 여전히 새어 나간다.

   - **큰 단위 + 작은 단위가 공백으로 이어지는 복합 표기** — `"3억 2천만원"`처럼 `억` 뒤에
     별도 숫자로 시작하는 `천만원` 구간이 이어지면, 정규식은 `원`에 가장 가까운 뒤쪽
     구간만 매치하고 앞의 `"3억"`은 원문에 남는다(실측: `"3억 2천만원"` →
     `"3억 <AMOUNT_1>"`, 매핑값 `"2천만원"`). `_AMOUNT_UNIT_RE`가 한 번의 매치 안에서
     처리하는 단위는 하나뿐이라, `억` 다음에 공백을 두고 다시 숫자+`만`이 나오는 구조를
     하나의 매치로 잇지 못하기 때문이다. `"1억 5,000만원"`도 같은 이유로
     `"1억 <AMOUNT_1>"`(매핑값 `"5,000만원"`)이 된다 — 이 형태는 한글 수사 패치 이전부터
     있던 것과 같은 종류의 간극이다.
   - **`원` 접미사가 전혀 없는 순수 `만`/`억` 표기** — `"3만"`, `"5억"`처럼 뒤에 `원`이
     붙지 않으면 세 정규식 중 어느 것도 매치하지 않아 **값 전체가** 원문에 남는다(실측
     확인). `_AMOUNT_UNIT_RE`는 `\s?원`으로 끝나야만 매치되므로 구조적으로 이 형태를
     다루지 않는다.
   - 추가로 `"2,000 만원"`처럼 숫자와 단위 사이에 공백이 있는 표기도 전량 마스킹되지
     않는다(실측 확인) — `_AMOUNT_UNIT_RE`의 `\s?`는 만/억과 원 사이에만 있고 숫자와
     단위 사이 공백은 다루지 않는다.

   세 경우 모두 이번 PR에서 고치지 않고 알려진 간극으로 남긴다.
