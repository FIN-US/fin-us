...
이쪽으로 옮기고 telegram_notifier는 재수출만 한다. 같은 이유로 설정값도 이 모듈이 끌어오지
않는다 — 임계값처럼 정책에 속하는 값은 인자로 받는다 (#308).
"""
...
def format_signal_score_line(
    ...
    *,
    ...
    인스턴스 없이 같은 문구를 쓸 수 있어야 한다. #308에서 telegram_notifier로부터
    이쪽으로 옮겼다: 사용자에게 나가는 문장을 조립하는 지점이 하나여야 한다는 #297의
    전제를 이 조각만 벗어나 있었다.
