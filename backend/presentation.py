...
def render(
    ...
    **길이 예산이 없다** (#313). #260부터 이 함수는 각주 자리를 뒤에서부터 확보하고
    ...
    문단을 쪼개지 않는다(:func:`_pack`). 각주 한 덩이의 길이는 위에서 이미 유계다 —
    추론 각주는 REASONING_FOOTNOTE_MAX_CHARS, 용어 각주는
    TERM_FOOTNOTE_MAX_ENTRIES줄 — 이므로 상한을 넘길 수 없고, 따라서 통째로 마지막
    조각(또는 각주만 담은 마지막 조각)에 실린다.
