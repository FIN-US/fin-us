...
**scope 충돌은 확률이 아니라 코드로 막는다.** 24비트(16^6 ≈ 1,678만 가지)에 동시 등록
수가 작아 확률은 무시할 만하지만, 확률만으로 닫으면 남는 실패 모드가 **조용한 오답**이다.
`_Counter.values`가 호출마다 `{"AMOUNT": 0, ...}`으로 시작하므로 두 매핑의 첫 자리표시자는
문자열까지 같다(`<AMOUNT_abc123_1>`). 그래서 덮어쓰기를 허용하면 겹치는 번호가 **남의
값으로 조용히 치환된다** — `unrestorable`은 비어 있고, 호출자는 거부할 근거를 받지
못한 채 다른 대화의 금액을 `Diary.content`에 저장한다. 실측(`token_hex` 고정):
...
이것은 이 모듈이 감수한 실패 모드가 **아니다.** 매핑을 전역 dict 하나로 올렸을 때의
폐해로 적어 둔 "다른 대화의 금액이 남의 일지에 저장된다" 그 자체다.

**늦게 온 쪽만 등록에서 빼는 것으로는 닫히지 않는다.** 두 자리표시자가 문자열까지 같아
등록소가 소유자를 구분할 수 없으므로, 늦게 온 쪽의 조회가 먼저 등록한 쪽의 매핑에 그대로
적중한다(방향만 뒤집힌 같은 오답이다). 그래서 :func:`active_mapping`은 충돌을 발견하면
그 자리를 :data:`_POISONED`로 덮어 **겹친 두 요청 모두** 되돌리지 못하게 한다 — 둘 다
`unrestorable`로 떨어져 호출자가 422로 거부한다.

멀쩡한 요청 하나를 함께 죽이는 거래다. 그래도 이쪽을 택한 것은 반대편이 **조용한
오답**이기 때문이고, #339가 "조용한 원본 소실 대신 시끄러운 실패"를 택한 것과 같은
판단이다. 이로써 "동시 호출이 서로의 매핑을 덮지 않는다"는 확률적 참이 아니라 **구조적
참**이 된다 — #354가 요구한 것이 그것이다.
(회귀: `test_pii_registry.py::TestScopeCollision`)
...
- **되돌리기는 인증을 켠 배포를 전제로 한다.** `create_db_diary`는 호출자를 구분하지 않고
  ...
  `main.py`의 `require_api_key`인데, `FINUS_API_KEY`를 비운 배포에서는 무인증으로 통과한다
  (새 설치만 setup_env가 키를 채우고, `_log_api_auth_state`가 꺼진 상태를 경고로 올린다). 실전 공격은 아니다 — scope는 24비트고
...
# scope 충돌이 난 자리에 대신 넣는 빈 매핑. 조회는 "scope는 있는데 그 번호가 없다"로
# 떨어져 되돌리기가 실패하고, 겹친 두 요청 모두 422로 거부된다. 빈 dict를 그냥 쓰지 않고
# 이름을 붙인 것은 의도를 grep으로 찾을 수 있게 하려는 것뿐이다 — 동작은 빈 dict 그대로다.
_POISONED: dict[str, str] = {}
...
# 정확히 한 줄이 남고, 등록소가 다시 빈 뒤에 재발하면 그때 다시 한 줄이 남는다.
#
# 재무장 조건이 "완전히 빔"이라는 것은 **이 경고가 잡으려는 진짜 누수가 있으면 다시
# 무장하지 않는다**는 뜻이다(빠져나오지 못한 항목이 남아 등록소가 영영 비지 않는다).
# 그 뒤의 독립적인 과부하 구간은 조용해진다. 동작을 바꾸지 않은 이유는 누수 자체가
# **첫 한 줄로 이미 보고됐기** 때문이다 — 같은 사실을 다시 알리려고 스팸 가능성을
# 되살릴 이유가 없다 (PR #361 리뷰).
_leak_warned = False
...
def active_mapping(mapping: dict[str, str]) -> Iterator[None]:
    ...
    # 이 호출이 **실제로 등록한** scope만 담는다. `grouped`를 그대로 지우면 충돌 상황에서
    # 늦게 온 쪽의 finally가 먼저 등록한 쪽의 매핑을 지운다 — 남의 요청을 죽인다.
    registered: list[str] = []

    with _LOCK:
        for scope, entries in grouped.items():
            if scope in _REGISTRY:
                # scope 충돌. **늦게 온 쪽만 빼는 것으로는 닫히지 않는다** — 두 매핑의
                # 자리표시자는 문자열까지 같으므로(`_Counter.values`가 호출마다 0에서
                # 시작한다) 등록소는 어느 요청의 토큰인지 구분할 수 없고, 늦게 온 쪽의
                # 조회가 먼저 등록한 쪽의 매핑에 그대로 적중한다. 그래서 **양쪽 모두**
                # 되돌리지 못하게 자리를 오염시킨다 — 둘 다 422로 거부된다.
