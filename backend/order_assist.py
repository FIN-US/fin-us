...
``/advise <종목명>`` 한 번, 또는 스케줄러 룰 트리거 한 번(#314)이 여기로 들어와 다섯 단계를
지난다. 두 계기는 :class:`ProposalTrigger`의 ``source``로만 갈리고, 아래 다섯 단계와 세 원칙은
완전히 같다 — 특히 자동 트리거라고 해서 확정 버튼을 건너뛰는 경로는 없다:
...
class ProposalTrigger:
    ...
    두 호출부가 같은 함수로 들어온다 — 텔레그램 수동 요청(``telegram_commands._handle_advise``)과
    스케줄러 룰 트리거(``scheduler.run_rule_triggered_proposal``, #314)다. 냉각 키가
    종목코드+룰 단위라, 룰이 없는 수동 요청과 룰 기반 자동 제안이 서로의 냉각을 잡아먹지 않는다.

    ``trigger_signal``에는 **수치를 넣지 않는다**. ``build_proposal_prompt``가 이 값을 프롬프트
    본문에 싣는데, ``/v1/propose-order``는 프롬프트 하나가 곧 transcript 전체라 본문에 실린
    수치가 환각 게이트(``fe_branch``)의 화이트리스트가 된다. 넣을 것은 "무엇을 확인하라"는
    지시 문구다 (``backend/order_rules.py`` 모듈 docstring 3번).
