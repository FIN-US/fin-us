...
def _looks_like_stock_code(stock: str) -> bool:
    ...
    이름·별칭 매칭보다 *먼저* 시도한다. 마스터에 없는 코드 형태 입력은 코드로 에코되므로,
    숫자를 포함한 6~7자 입력에 대해서는 JS도 결국 입력과 같은 코드를 돌려준다. 다만 이
    함수는 마스터를 볼 수 없어 존재 검증 없이 코드로 확정한다(#151).
...
async def _resolve_stock_code(stock: str) -> str:
    ...
    try:
        ...
        # 으로도 없을 때만 나므로 실제 마스터 데이터로는 발생하지 않는다. #174에서 코드
        # 완전일치가 이름·별칭 매칭보다 먼저로 바뀐 뒤에도 이 결론은 그대로다. 게다가 위
        # 지름길(_looks_like_stock_code)이 숫자를 포함한 6~7자 입력을 MCP에 보내지 않아,
        # 여기 도달할 수 있는 에코는 숫자 없는 코드뿐이라 _has_code_digit이 잡는다.
