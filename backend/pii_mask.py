...
# **않았다**. 근거는 아래 "조 복합 표기" 문단에 적었다(마디 연쇄를 {1,3}으로 묶어
...
#
# 조 복합 표기 — 첫 갈래를 {1,3}으로 묶고 단위 갈래에 "조"를 넣는 이유. (#330)
...
#
# 한국어 큰 수 표기는 조 > 억 > 만 순으로 각 단위가 많아야 한 번씩이므로("12조3456억
# 7890만원") 마디 상한은 3이다. 무한 반복(+)이 아니라 상한으로 묶는 것이 성능 요건이다:
# +로 두면 "1조1조1조…"처럼 마디가 이어지는 런에서 각 시작 위치가 런 전체를 소비했다가
# \s?원에서 실패해 O(n) 시작 x O(n) 소비 = 위 문단과 같은 O(n²)로 되돌아간다. {1,3}은
# 시작 위치당 소비를 세 마디로 묶어 전체를 선형으로 유지한다 — 그래서 lookbehind 문자
# 집합에 "조"를 넣지 않아도 된다(실측: 8192자 '1조' 반복 mask_pii 3.45 ms -> 5.77 ms,
# '1억' 반복 3.95 ms -> 6.77 ms, '1천조' 반복 4.73 ms -> 7.53 ms — 상수배 차이이고
# 회귀 테스트 상한 0.5 초의 1/60 수준이다).
...
# 성능 요건이고, 이쪽은 상한 3으로도 막히지 않는다. 마디를 이어 붙이면 "1천1천1천…"
...
#
# main 대비 차분: 접두 13종 x 금액표현 46종 x 접미 8종 = 4784조합 스윕에서 부분 마스킹이
# 전량 마스킹으로 바뀌거나 새로 잡힌 것 1120건, **마스킹이 사라진 것 0건, 새 과탐 0건,
# 왕복 손실 0건**. 여전히 남는 간극은 "조"/"억" 뒤에 **공백**을 두고 이어지는 표기
# ("9조 1800억원"·"3억 2천만원")로, docs/nfr-05-pii-masking.md 미적용 경로 5번에 있다.
_HANGUL_DIGIT = r"[일이삼사오육칠팔구]"
_AMOUNT_UNIT_RE = re.compile(
    r"(?<![\d,천백십일이삼사오육칠팔구])(?:"
    r"(?:\d+(?:,\d{3})*(?:\.\d+)?(?:(?:[천백십]\d*)++(?:조|억|만)?|(?:조|억|만))){1,3}"
