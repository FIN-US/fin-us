...
# 접근 제어다. 키가 설정된 배포에서만 걸린다 — 키를 비운 배포에서는 무인증이고, 새 설치만
# setup_env가 키를 채우는 이유는 config.FINUS_API_KEY 주석에 있다.
#
...
def _log_api_auth_state() -> None:
    ...
    키를 비워 둔 기존 배포는 아무 증상 없이 동작하므로 조용히 지나가기 쉽다. 무인증으로 열려 있다는 사실은
