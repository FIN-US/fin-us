...
def test_strip_trims_only_outer_whitespace_before_shape_check():
    # 공유 판정표(orderable_code_policy.json)는 strip 없이 정규식만 구동하므로, 앞뒤
    # 공백을 걷어내는 이 함수의 전처리는 여기서 따로 고정한다. 바깥 공백은 걷히고
    # 내부 공백은 남아 형태 검사에서 걸려야 한다.
    assert check_orderable_code(" 005930 ", frozenset()) is None

    violation = check_orderable_code(" 005 930 ", frozenset())
...
def test_suggestion_path_stays_numeric_only_even_when_alnum_flag_is_on(monkeypatch):
    ...
    6~7자)에 묶어 둔다. KIS가 영숫자·9자 PDNO를 실제로 수용하는지 확정되기 전(#265
    실측)까지 유지한다. 나중에 이 비대칭을 없앨 때는 이 테스트를 함께 지워야 한다 —
    조용히 갈리지 않도록 여기서 고정한다.
    """
