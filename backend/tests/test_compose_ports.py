"""docker-compose가 무인증 서비스를 전 인터페이스에 게시하지 않는지 고정한다(#285·#246).
...
`backend`도 #246에서 같은 자리에 합류했다 — 인증이 없고 `/api/v1/analyze` 한 번이 LLM
과금으로 이어지는데, 전 인터페이스에 게시돼 있으면 nginx에 건 레이트리밋(#266 1단계)이
8000 직접 호출로 그대로 우회된다. 이 파일이 그 우회로가 다시 열리는 것을 막는다.
...
#   - frontend(8080): 대시보드를 다른 기기에서 여는 것이 사용 목적 그 자체다. 무인증인
#     것은 같지만, 여기가 레이트리밋을 거는 지점이기도 하다(frontend/nginx.conf).
#
# `backend`는 #246에서 빠졌다. 번들이 `http://localhost:8000` 하드코딩을 버리고 상대
# 경로로 재빌드되면서(#262) 원격 시연이 8000 직접 호출에 기대지 않게 됐고, 그 뒤로
# 전 인터페이스 게시는 nginx 레이트리밋의 우회로일 뿐이다. 되돌리려면 그 우회 문제를
# 먼저 풀 것.
_INTENTIONALLY_PUBLIC = frozenset({"frontend"})
