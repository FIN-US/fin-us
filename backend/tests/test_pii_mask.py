...
class TestAmountRecognizer:
    ...
    def test_masks_composite_big_unit_amount(self):
        ...
        - 마디 연쇄 {1,4} 제거(마디 하나만 허용): "조" 갈래는 살아 있어 "10조원"은
          통과하지만 "9조1800억원"이 "9조<AMOUNT_1>"로 되돌아간다.
        - 상한을 {1,3}으로 낮춤: 단위 없는 꼬리 수사 마디가 밀려나
          "1조5000억3000만5천원"이 "1조<AMOUNT_1>"로 부분 마스킹된다.
        - lookbehind 문자 집합에 "조" 추가: 마디 연쇄에 상한이 있어 성능상 필요 없는
          변경인데, "수조5000억원"처럼 "조" 앞에서 매치를 시작할 수 없는 표기는 뒤
          구간마저 시작이 막혀 값 전체가 평문이 된다(마지막 대조군 루프가 FAILED).
        - lookbehind 문자 집합에 "제" 추가: 아래 대조군에 적은 "제N조" 과탐을 막으려는
          변경인데, 같은 이유로 "제1조원"이 전량 평문이 되고 "제1조5000억원"이 부분
          마스킹으로 되돌아간다(마지막 대조군 루프가 FAILED).
        ...
        for text, expected, original in [
            ...
            # + 단위 없이 수사 런만으로 성립하는 꼬리 마디 = 연쇄 상한 4.
            # 상한이 3이면 "1조<AMOUNT_1>"로 부분 마스킹된다.
            ("1조5000억3000만5천원", "<AMOUNT_1>", "1조5000억3000만5천원"),
            ("1조2억3만4천원", "<AMOUNT_1>", "1조2억3만4천원"),
            ("1억2천만3천원", "<AMOUNT_1>", "1억2천만3천원"),
        ...
        for text, expected in [
            ...
            ("조원장님께 보고", "조원장님께 보고"),
            ("천조국이라 불린다", "천조국이라 불린다"),
            # 감수하는 과탐: 법조문 서수사 "제N조" 뒤에 "원"으로 시작하는 단어가 오면
            # \s?원이 그 첫 음절을 삼킨다(main에서는 불변이었다). 왕복 무손실이고
            # _AMOUNT_UNIT_RE 주석이 이미 감수한 "이백 원자재"와 같은 급·같은 원인이라
            # 고치지 않는다 — lookbehind에 "제"를 넣으면 "제1조원"이 전량 평문이 되어
            # "조"에 대해 기각한 교환이 그대로 재현된다. 방향이 바뀌면(고치든, 더
            # 번지든) 이 줄이 알려준다.
    ...
    def test_masks_digit_tail_after_unit_marker(self):
        r"""단위 뒤에 아라비아 숫자만 오는 표기도 값 전체가 하나의 자리표시자여야 한다.

        "1만5000원"류(#333)다. 위 test_masks_composite_big_unit_amount의 부분 마스킹과
        같은 클래스지만 원인이 다르다 — 그쪽은 정규식이 조 단위 표기를 몰라서였고,
        이쪽은 알면서 **적용 순서** 때문에 놓쳤다. mask_pii는 이미 치환된 텍스트를 다음
        정규식에 넘기므로, 좁은 _AMOUNT_WON_RE("원" 바로 앞 숫자)가 먼저 돌면 값의 꼬리
        "5000원"만 잘라가고 앞자리 "1만"이 평문으로 남는다. 꼬리가 한글 수사인 "1만5천원"은
        #330의 마디 연쇄로 이미 전량 마스킹되고 아라비아 숫자 꼬리만 남아 있었다.

        "1만5000원"·"7만8000원"은 주가·체결가 표기로 흔하고 telegram_commands의
        _handle_chat_fallback이 사용자 원문을 가공 없이 넘기므로, 뉴스 본문뿐이던 #330과
        달리 F-17 보호 대상인 금액의 앞자리가 실제로 외부 LLM에 나갔다.

        고치는 데 순서 교체와 꼬리 마디가 **둘 다** 필요하다. 어느 하나만 되돌려도
        main의 부분 마스킹이 그대로 돌아온다 — 이 테스트가 잡는 mutation(실측,
        입력 "1만5000원"):

            현재                     "<AMOUNT_1>"      매핑 "1만5000원"    PASS
            M1 적용 순서 되돌림       "1만<AMOUNT_1>"   매핑 "5000원"      FAILED
            M2 꼬리 마디 제거         "1만<AMOUNT_1>"   매핑 "5000원"      FAILED
            M1+M2 (= main)          "1만<AMOUNT_1>"   매핑 "5000원"      FAILED

        M1은 _AMOUNT_WON_RE가 먼저 꼬리 "5000원"을 잘라가서, M2는 "5000"이 마디가 되지
        못해(마디는 수사 런이나 단위를 요구한다) _AMOUNT_UNIT_RE가 값 전체를 놓치고 뒤에
        도는 _AMOUNT_WON_RE가 다시 꼬리만 잘라가서다. 원인이 다르고 결과가 같다.

        꼬리 앞 공백 (?:(?<=[만억])[ \t])?도 같은 유출을 닫는다 — "1만 5000원"은 자유
        입력에서 붙임 표기보다 오히려 흔한데 main에서 "1만 <AMOUNT_1>"이었다. 이 조건의
        세 부분이 각각 필요하다(실측):

            M3 공백 자체를 제거        "1만 5000원" -> "1만 <AMOUNT_1>"        FAILED
            M4 [ \t] 대신 \s          "잔고는 1만\n5000원"이 한 값으로 합쳐짐   FAILED
            M5 lookbehind에 "조" 추가  "제3조 5000원 이하의 과태료" ->
                                      "제<AMOUNT_1> 이하의 과태료"            FAILED
            M6 lookbehind 자체를 제거  M5와 같은 자리에서 FAILED

        M5·M6이 막는 것이 "조"를 넣지 않는 이유다. 조문 번호가 별개 금액과 한 값으로
        합쳐져 프롬프트에서 사라진다 — 아래 대조군에 그 자리를 고정한다.

        대조군은 마지막 루프다. 순서를 뒤집었으므로 _AMOUNT_WON_RE만의 영역(단위 없는
        "원" 표기)이 여전히 잡히는지, 꼬리 마디가 금액 아닌 숫자를 새로 삼키지 않는지를
        함께 고정한다.
        """
        for text, expected, original in [
            ("1만5000원", "<AMOUNT_1>", "1만5000원"),
            ("7만8000원", "<AMOUNT_1>", "7만8000원"),
            ("10만9900원", "<AMOUNT_1>", "10만9900원"),
            ("1천5000원", "<AMOUNT_1>", "1천5000원"),
            ("3억5000원", "<AMOUNT_1>", "3억5000원"),
            ("1조5000원", "<AMOUNT_1>", "1조5000원"),
            # 꼬리 마디는 단위로 끝난 마지막 마디 뒤에만 붙는다 — 연쇄와 함께 쓰이는 형태.
            ("5억3000만2000원", "<AMOUNT_1>", "5억3000만2000원"),
            ("1조5000억3000만2000원", "<AMOUNT_1>", "1조5000억3000만2000원"),
            # 꼬리의 숫자 모양은 마디 선두와 같다(콤마·소수 허용).
            ("1만5,000원", "<AMOUNT_1>", "1만5,000원"),
            ("1만5000.5원", "<AMOUNT_1>", "1만5000.5원"),
            # 단위 뒤에 공백을 두는 표기도 같은 값이다. 꼬리 앞 공백을 "만"/"억" 뒤로
            # 좁혀 허용한다((?<=[만억])[ \t]) — 자유 입력에서는 붙임 표기보다 흔하다.
            ("1만 5000원", "<AMOUNT_1>", "1만 5000원"),
            ("1억 5000원", "<AMOUNT_1>", "1억 5000원"),
            ("10만 9900원", "<AMOUNT_1>", "10만 9900원"),
            ("1만 5,000원", "<AMOUNT_1>", "1만 5,000원"),
            ("1천만 5000원", "<AMOUNT_1>", "1천만 5000원"),
            ("잔고 1만 5000원인데", "잔고 <AMOUNT_1>인데", "1만 5000원"),
            # 자유 입력 경로의 실제 표기
            ("잔고 1만5000원인데 뭘 살까요", "잔고 <AMOUNT_1>인데 뭘 살까요", "1만5000원"),
            ("주가가 7만8000원까지 올랐다", "주가가 <AMOUNT_1>까지 올랐다", "7만8000원"),
        ]:
            masked, mapping = mask_pii(text)
            assert _norm(masked) == expected, (
                f"단위 뒤 숫자 꼬리가 부분 마스킹됐다: {text!r} -> {masked!r}"
            )
            assert _norm_mapping(mapping) == {"<AMOUNT_1>": original}, (
                f"매핑 원값이 값 전체가 아니다: {text!r} -> {mapping!r}"
            )
        ...
        for text, expected in [
            # _AMOUNT_WON_RE만의 영역 — 단위가 없어 _AMOUNT_UNIT_RE는 매치하지 않는다.
            # 순서를 뒤집어도 이쪽이 마스킹되지 않으면 그 자체가 유출이다.
            ("1,234,567원", "<AMOUNT_1>"),
            ("15000원", "<AMOUNT_1>"),
            ("5000원", "<AMOUNT_1>"),
            ("1234567890원", "<AMOUNT_1>"),
            # 꼬리 앞 공백은 "만"/"억" 뒤로만 허용한다. "조"까지 넣으면 법조문 서수사가
            # 값에 삼켜진다("제3조 5000원 이하의 과태료" -> "제<AMOUNT_1> 이하의 과태료",
            # 매핑 "3조 5000원"). 그래서 "조" 뒤 공백 표기는 알려진 간극으로 남는다.
            ("1조 5000원", "1조 <AMOUNT_1>"),
            ("제3조 5000원 이하의 과태료", "제3조 <AMOUNT_1> 이하의 과태료"),
            # 공백 뒤가 순수 숫자가 아니라 수사·단위를 가진 마디면 꼬리가 받지 못한다
            # (docs/nfr-05-pii-masking.md 미적용 경로 5번의 알려진 간극 그대로).
            ("9조 1800억원", "9조 <AMOUNT_1>"),
            ("3억 2천만원", "3억 <AMOUNT_1>"),
            # [ \t]는 개행을 넘지 않는다. \s로 두면 줄 끝의 "1만"과 다음 줄 첫머리의
            # "5000원"이 한 자리표시자로 합쳐져, 서로 다른 두 값이었다는 사실이 사라진다.
            ("잔고는 1만\n5000원", "잔고는 1만\n<AMOUNT_1>"),
            # 공백이 둘 이상이거나 단위와 공백 사이에 다른 문자가 끼면 붙지 않는다.
            ("1만  5000원", "1만  <AMOUNT_1>"),
            ("1만, 5000원", "1만, <AMOUNT_1>"),
            # 단위 뒤 공백 다음이 금액이 아닌 경우
            ("관객 1만 5000명 원정대", "관객 1만 5000명 원정대"),
            ("10만 명이 5000원", "10만 명이 <AMOUNT_1>"),
            # 감수하는 과탐 — 기존 "제N조" 과탐이 조문 번호까지 삼키도록 넓어졌다.
            # main은 "제3조<AMOUNT_1>"(매핑 "5000원")이었다. 꼬리 마디가 붙으면서 값이
            # "3조5000원"으로 길어진다. 왕복 무손실이고 #330이 이미 기록·감수한 과탐과
            # 같은 클래스라 감수하되, 방향이 바뀌면 이 줄이 알려준다.
            ("제3조5000원", "제<AMOUNT_1>"),
            ("제3조1000원짜리", "제<AMOUNT_1>짜리"),
            # 금액이 아닌 숫자를 새로 삼키지 않는다("원"이 없거나 단위 마디가 없다).
            ("1만2000명 원정대", "1만2000명 원정대"),
            ("12만3456번 원고", "12만3456번 원고"),
            ("제1조2항 원안 가결", "제1조2항 원안 가결"),
            ("주문번호 20240101 원장", "주문번호 20240101 원장"),
            ("코스피 2500 원달러 1300", "코스피 2500 원달러 1300"),
        ]:
            masked, mapping = mask_pii(text)
            assert _norm(masked) == expected, (
                f"대조군 동작이 바뀌었다: {text!r} -> {masked!r}"
    ...
    def test_unit_and_bare_won_amounts_coexist_losslessly(self):
        """두 금액 정규식이 한 텍스트에 섞여도 각자 값 전체를 가져가고 왕복이 무손실이다.

        #333의 순서 교체가 남기는 눈에 보이는 변화는 자리표시자 **번호**다. mask_pii는
        정규식별로 한 번씩 훑으므로 번호는 텍스트 등장 순서가 아니라 훑는 순서를 따른다 —
        "3000원 그리고 1만5000원"의 <AMOUNT_1>은 이제 뒤에 있는 "1만5000원"이다.

        이것은 이 다중 패스 설계에 원래 있던 성질이지 이번 변경이 만든 것이 아니다.
        main에서도 "잔고 1234567 그리고 3천만원"은 라벨 패스가 나중이라 <AMOUNT_2>가
        앞에 온다(실측으로 main과 현재가 동일). 번호는 값의 **동일성 표지**일 뿐이고,
        모듈 docstring이 보존한다고 선언한 능력(자리표시자끼리의 상대 비교)도 번호의
        크기가 아니라 서로 다른 토큰이라는 사실에 기대므로 영향받지 않는다.

        이 테스트가 고정하는 것은 그래서 번호의 순서가 아니라 **매핑과 왕복**이다:
        어떤 순서로 번호가 붙든 각 자리표시자가 값 전체를 담고 역치환이 원문을 복원한다.
        (위 test_masks_digit_tail_after_unit_marker가 잡는 mutation 셋은 여기서도 RED다 —
         "2만3000원"의 매핑이 "3000원"으로 잘려 첫 단언에서 FAILED.)
        """
        text = "예수금 1,234,567원, 평가금액 2만3000원, 삼성전자 10주 보유"
        masked, mapping = mask_pii(text)

        assert set(mapping.values()) == {"1,234,567원", "2만3000원", "10"}
        # 원문 숫자가 어떤 형태로든 마스킹된 텍스트에 남아 있으면 안 된다.
        for leaked in ["1,234,567", "2만", "3000", "1,234", "567"]:
            assert leaked not in masked, (
                f"{leaked!r}가 마스킹된 텍스트에 평문으로 남았다: {masked!r}"
            )
        assert unmask_pii(masked, mapping) == text
    ...
    def test_chained_unit_run_does_not_blow_up(self):
        ...
        **시작 위치**가 O(n)개라 생기는 문제이고 lookbehind로 막는다. 이쪽은 마디 연쇄
        {1,4}를 넣으면서 생긴 것으로, "1천1천1천…" 같은 런을 "한 마디의 긴 수사 런"으로도
        ...
        assert elapsed < 0.5, (
            ...
            "걸렸다 — 마디 연쇄 안의 수사 런이 소유 수량자로 확정되지 않아 "
