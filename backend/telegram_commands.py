...
class TelegramCommandHandler:
    ...
    async def _answer_callback_query(
        ...
        *,
        ...
        """콜백 답. ``text``가 있는 답이 실패하면 같은 문구를 메시지로 대체 전송한다 (#382).

        ``text``를 싣는 호출은 모두 **그 답이 사용자에게 가는 유일한 통지인 분기**다 — 주문
        버튼의 저장소 오류·만료 버튼(ORDER_STALE_CALLBACK_TEXT), 조회 버튼의 만료
        (MARKET_STALE_CALLBACK_TEXT, 토큰 없음·채팅 불일치), 그리고 "지원하지 않는 버튼입니다"
        (알 수 없는 콜백·/alerts·/level·거래·조회 메뉴의 알 수 없는 동작). 모두 답을 한 뒤
        ...
        if not callback_query_id:
            return
        answered = await self.notifier.answer_callback_query(callback_query_id, text=text)
        if answered is False and text:
            await self._send_text_settled(text)
        ...
        try:
            ...
            if isinstance(exc, HTTPException) and exc.status_code == 403:
                ...
                if restored:
                    ...
                    #
                    # 이 안전은 프로세스 안의 재시도에만 해당한다. 재시작을 넘긴 재시도(예산은
                    # #350으로 살아남는다)는 owner가 달라 #383 표지에 걸려, 사유 대신 재실행
                    # 안내로 끝난다. 실행하지 않는 쪽이라 안전하고 복원된 주문은 그대로 남는다.
    ...
    async def _handle_chat_fallback(self, text: str, chat_id: str) -> None:
        ...
        try:
            ...
            # 통지보다 먼저 치운다 — 아래 전송이 끝내 실패해도 "분석 중"이 채팅에 남지
            # 않는다 (#260).
            await self._clear_progress_message(progress_message_id)
            # 실패 통지도 답변과 같은 settled 전송이다 (#259 4단계, #275). 재시도 가능한
            # 전송으로 두면 전송 실패가 update 재시도로 번져 이 메서드가 처음부터 다시 돈다 —
            # 진행 메시지가 새로 나가고 LLM이 다시 호출된다(실패가 타임아웃이면 이미 과금됐을
            # 수 있다). 대가는 LLM 실패와 통지 전송 최종 실패가 겹칠 때 사용자가 실패 사실을
            # 못 받는 것인데, 주문 상태를 오인할 메시지가 아니고 재질문으로 복구된다. 그 한
            # 건은 settled_send 메트릭에 남는다.
            await self._send_text_settled(f"응답 생성 실패: {_short_error(exc)}")
            return
