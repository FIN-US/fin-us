...
// 유량 제한 msg_cd. 세 코드를 본다.
// - EGW00201: 한국투자증권 공식 GitHub README가 "초당 거래건수 초과"로 명시한 코드
//   (index.js 상단 조사 주석 참조). 2026-09-22 모의투자 실측(#210)에서 14건 관측 —
//   msg1 "초당 거래건수를 초과하였습니다.", HTTP 500 + rt_cd=1. 계정 전체 호출률이
//   초당 1~2건을 넘을 때 났다.
// - EGW00133: 같은 계열의 유량 제한 코드. 지금은 tests/kis-client.test.js의 픽스처 문구로만
//   저장소에 존재하고 프로덕션 코드 어디에서도 감지되지 않는다.
// - EGW00215: 2026-09-22 모의투자 실측(#210, #410)에서 관측한 코드 — msg1 "원장에서 허용
//   가능한 초당 거래건수를 초과하였습니다.", HTTP 500 + rt_cd=1. EGW00201과 달리 계정 호출률이
//   낮을 때도(직렬 호출, 초당 1건 미만) 났다. 지금은 msg1의 "초당 거래건수"로도 잡히지만,
//   그 문구가 바뀌면 이 코드가 class=other로 떨어지고 게이트가 꺼진 기본 설정에서는 줄이
//   아예 안 나가므로 코드로도 고정한다(런북 7절 "실측에서 나온 코드는 이 목록에 추가").
export const KIS_RATE_LIMIT_MSG_CODES = Object.freeze(["EGW00201", "EGW00133", "EGW00215"]);
