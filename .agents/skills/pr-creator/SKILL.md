...
- 본문 전체 **1,500자**를 상한으로 둡니다.
- **diff에 있는 코드를 본문에 옮기지 않습니다.** 코드 인용은 diff만 봐서는 안 보이는 것
  (제거된 동작, 호출 관계, 재현 절차)일 때만, 3줄 이내로 씁니다.
- **검증 결과는 수치만 적습니다.** `pytest backend/ — 469 passed, 2 skipped` 한 줄이면 됩니다.
  뮤테이션 표·시도별 기록처럼 재현 가능한 과정은 넣지 않습니다.
- 섹션마다 요지를 반복하지 않습니다. 같은 사실은 본문에 한 번만 나옵니다.
- 알려진 한계나 범위 밖 항목은 **각 한 줄**로 적습니다. 없으면 그 줄도 쓰지 않습니다.
